...
public class HostController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        Model.getInstance().getViewFactory().getHostMenuOptions().addListener(((observableValue, oldVal, newVal) -> {
            switch (newVal){
                case HOME -> user_parent.setCenter((Model.getInstance().getViewFactory().getHomeView_host()));
                //case DASHBOARD -> user_parent.setCenter((Model.getInstance().getViewFactory().getDashboardView()));
                case FIND -> user_parent.setCenter((Model.getInstance().getViewFactory().getFindView_host()));
                ...
                case USER_PROFILE -> user_parent.setCenter((Model.getInstance().getViewFactory().getUserProfileView_host()));
                ...
                default ->  user_parent.setCenter((Model.getInstance().getViewFactory().getHomeView_host()));
            }
        }));
    }
}